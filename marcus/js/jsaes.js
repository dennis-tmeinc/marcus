 ...
 */
 ...
 /* modified by Dennis Chen @ TME */
 
function aes_crypt( text, password, enc) {
...
function bin_hex( bin )
{
	...
	var hex = '' ;
	for( var i=0; i<l ; i++ ) {
		hex += ('0' + bin[i].toString(16)).substr( -2 );
	}
	return hex ;
}
...
function genkey( password )
{
	...
	for( var i =0 ;i<l ; i++ ) {
		key[i%32] += password.charCodeAt( i ) ;
		key[i%32] %= 256 ;
	}
    AES_ExpandKey(key);
	return key ;
...
function _decrypt( ctext, key ) 
{
	...
	l = ctext.length ;
	while( s < l ) {
		var block = hex_bin( ctext.substr( s, 32) );
		s+=32 ;
		...
		for( i=0; i<16; i++ ) {
			if( block[i] == 0 ) {
				...
				dec += String.fromCharCode( block[i] );
			}
		}
	}
